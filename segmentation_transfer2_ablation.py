...
# model = torch.load('./best_model_t2.pth')   
...
#     dict(params=filter(lambda p: p.requires_grad, model.parameters()), lr=0.00005),
# ])
...
for epoch in range(10):
    ...
    if average_precision > max_ap:       
        np.save('t2_t1_label_transfer1_self_learning.npy',labels_out)
        np.save('t2_t1_score_transfer1_self_learning.npy',scores_out)    
