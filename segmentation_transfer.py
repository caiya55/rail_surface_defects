...
model = torch.load('./best_model_t1_notran.pth')    
...
optimizer = torch.optim.Adam([ 
    dict(params=filter(lambda p: p.requires_grad, model.parameters()), lr=0.000001),
])
...
for epoch in range(5):
    ...
    if average_precision > max_ap:       
        np.save('t1_t2_label_transfer1au_all.npy',labels_out) 
        np.save('t1_t2_score_transfer1au_all.npy',scores_out)    
        max_ap = average_precision
        torch.save(model, './best_model_transfer_for_t1.pth')
        
        
'''type II self training'''
